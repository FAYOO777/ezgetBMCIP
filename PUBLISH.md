...
.\scripts\publish-local-branch.ps1                 # 本地支线：自动读取 Version.props，生成 Full + Legacy 双包
```

执行后生成 `README.txt`（最近 5 条 commit），可手动补充本次改动摘要。

`publish-local-branch.ps1` 不依赖 Git tag，专用于尚未合并或发布的本地支线。它只会在
`publish\local-branch\v<版本号>\` 创建一个新的目录；若目录已存在会失败，绝不覆盖既有发布物。
构建先在临时目录中完成，只有全部成功后才生成最终版本目录；失败时会清理临时产物。
该脚本生成 Win10/11 x64 自包含单文件包和 Win7 SP1/8/8.1 的 .NET Framework 4.6 Legacy 多文件包，
并写入 `SHA256SUMS.txt`。它不会创建 Git tag、推送、上传或修改线上版本索引。

## 版本号规则

- 当前软件版本统一由根目录 `Version.props` 中的 `EzGetBmcIpVersion` 定义；正式版、Legacy 和两个 Core 工程共用该值。
- 当前本地支线从 `2.0.0` 开始。完成一轮会影响用户运行软件的修改时，必须同步递增版本号，再进行构建或本地打包。
- 版本号按十进制逐位进位：`2.0.0 → 2.0.1 → … → 2.0.9 → 2.1.0 → … → 2.9.9 → 3.0.0`。
- 纯文档、测试记录、日志分析、网站内容或不改变软件产物的发布流程维护不递增软件版本；混合批次只按软件改动递增一次。
- 本地支线版本升级不代表正式发布：不创建 Git tag、不推送、不上传，也不修改线上 `versions.json`。
- `publish-local-branch.ps1` 会校验命令行版本与 `Version.props`；两者不一致时停止，避免包名和程序集版本分叉。

完整判断边界见 [`docs/versioning.md`](docs/versioning.md)。
...
| 8 | DHCP 分配后确认地址可达；HTTPS/HTTP 端口可用时打开浏览器 | ☐ | ☐ | ☐ | ☐ |
...
| BMC IP 显示 | IP 地址显示正确 | | ☐ |
| 打开 BMC | TCP 443/80 可用时按 HTTPS 优先打开，网页结果不作成功门槛 | | ☐ |
