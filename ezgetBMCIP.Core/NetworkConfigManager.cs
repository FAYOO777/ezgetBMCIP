...
public static class NetworkConfigManager
{
    ...
    {
        var details = await RestoreDhcpAndCollectLogAsync(adapter, cancellationToken, releaseToolLease);
        for (var i = 0; i < 10; i++)
        {
    ...
    private static async Task<string> RestoreDhcpAndCollectLogAsync(WiredAdapter adapter, CancellationToken cancellationToken, bool releaseToolLease)
    {
