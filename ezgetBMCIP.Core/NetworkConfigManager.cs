...
public static class NetworkConfigManager
{
    ...
    public static async Task RestoreOriginalConfigAsync(WiredAdapter adapter, AdapterOriginalConfig origConfig, SubnetConfig subnetConfig, CancellationToken cancellationToken)
    {
        ...
        {
            await RunNetshAsync($"interface ipv4 set address name=\"{adapter.Name}\" source=static", cancellationToken);
        }
        ...
        bool requireToolLeaseRemoved = true)
    {
        ...
        var gatewayAddressesMatch = expected.DhcpEnabled || IpSetEquals(current.Gateways, expected.Gateways);
        var gatewayMetricsMatch = expected.DhcpEnabled || expected.GatewayMetrics.Count == 0
        ...
        var dnsServersMatch = expected.DnsServersFromDhcp || IpSetEquals(current.DnsServers, expected.DnsServers);
        ...
        var details = $"success={success}, mode={current.DhcpEnabled}/{expected.DhcpEnabled}, " +
    ...
    private static RegistryKey? OpenAdapterRegistryKey(WiredAdapter adapter)
    {
        ...
        {
            var key = Registry.LocalMachine.OpenSubKey(basePath + name, false);
            if (key is not null)
                return key;
        }
        return null;
