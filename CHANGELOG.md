## ezgetBMCIP 更新日志

### v2.0.5（本地支线）
- 地址可达性检测不会再因 Ping 或 TCP 80 先成功而提前结束；在现有 5 秒窗口内继续等待 TCP 443，检测到后优先使用 HTTPS。
- 结果页改为准确描述 TCP 443/80 端口可连接，不再把 TCP 握手表述为 HTTP/HTTPS 页面已经连接。
- 仅 TCP 80 可连接时增加“尝试 HTTPS”入口，便于 BMC 的 HTTPS 服务在探测窗口后启动时直接切换；正式版与 Legacy 行为一致。

### v2.0.4（本地支线）
- 地址验证页按实际探测结果显示 Ping、HTTPS 和 HTTP 状态；仅管理端口连接成功时才显示管理页面入口。
- HTTPS 与 HTTP 都连接时默认使用 HTTPS，并将“改用 HTTP”收进其他方式；仅 Ping 或无响应时只提供重新检测和复制地址。
- 运行页移除诊断详情折叠区，浏览器状态改为“已交给系统浏览器打开”或“未能打开系统浏览器”。
- 正式版、Lite 和 Legacy 保持同一套事实表达，不改变 DHCP、探测、浏览器请求或网卡恢复流程。

### v2.0.3（本地支线）
- 正式版、Lite 和 Legacy 的网络修改风险告知移除“开始前检查”清单，改为“本次会发生什么 → 必要时显示防火墙风险 → 退出后如何恢复”。
- 防火墙正常时保持安静；Warning、Unknown 和 High 只显示与证据强度相符的提示，并保留可展开的完整诊断详情。
- 不改变预检查、防火墙判断、网卡配置、DHCP、恢复流程或确认行为。

### v2.0.2（本地支线）
- 修复 Legacy 防火墙处理成功后仍显示黄色“防火墙需要留意”和重复处理按钮的问题。
- Legacy 现在根据复查结果显示“防火墙许可已处理”或“防火墙处理结果”，仅在仍有风险时提供重新处理入口。
- Full 和 Lite 共用的正式版防火墙结果展示保持现有动态行为；防火墙规则、网卡恢复和 DHCP 流程不变。

### v2.0.1（本地支线）
- DHCP 等待超时且防火墙评估为 Warning 或 Unknown 时，恢复“处理防火墙并准备重试”入口。
- 保留不确定性提示；点击后仍会重新检查、展示处理范围并等待用户明确确认，不改变防火墙判断或处理范围。

### v2.0.0（本地支线）
- 将从 v1.5.1 起逐步完成的 DHCP、防火墙、历史网段、安全恢复、端点检测及 Modern/Legacy UI 重构统一收敛为 2.0.0 基线。
- 正式版、Legacy 和共享 Core 工程改为从根目录 `Version.props` 读取同一版本号。
- 后续每轮影响用户运行软件的修改完成后按十进制规则递增末位；纯文档、测试记录和发布规则维护不递增软件版本。本地支线仍不创建正式 tag、不上传发布物或修改线上版本索引。

### v1.5.8（本地测试）
- DHCP ACK 后改为最多约 5 秒并行检测 Ping、TCP 443 和 TCP 80；地址可达即完成，不再等待 HTTP/TLS 页面响应。
- 443/80 均不可连接时保留候选 IP 并提示重新检测；有端口连接时按 HTTPS 优先自动打开浏览器。
- 修复 DHCP ACK 后 Windows 网卡重新枚举造成的“无法重新找到所选网卡”误失败。
- 端点验证加入约 2 秒短时重试，并按规范化 GUID、MAC、唯一名称+描述安全解析；所有候选都必须拥有本次临时 IPv4。
- 无法唯一确认时保留候选地址，失败页提供重新验证、导出支持包和恢复退出，现代版与 Legacy 行为一致。

### v1.5.7（本地测试）
- 管理服务确认升级为所选直连网卡上的路由、ARP、TCP、TLS 和最终 HTTP/HTTPS 管理响应验证（2xx、3xx、401、403）；不再将单纯端口连接、HTTP 1xx、代理 407 或服务端 5xx 视为网页可访问。
- `.100` 旧租约回退与 DHCP 保持并行，只有完成协议响应验证才会获胜；旧 TCP-only 历史自动失效。
- 防火墙处理可区分 Windows 自动生成的同名 TCP/UDP Block 规则，只修复当前 EXE 的相关 UDP 入站阻止。

### v1.5.6（本地测试）
- Main / Legacy 新增经用户确认的防火墙处理入口，恢复网卡后限定当前 EXE、接口及网络类型放行 UDP/67；冲突规则逐项告知，策略限制拒绝处理，失败回滚，成功后等待手动重试。
- 首次 DHCP 超时后，可基于同一物理网卡上次成功记录，恢复网卡并预填旧网段供用户手动重试。
- BMC HTTP/HTTPS 管理端口确认可访问后，在本机保存一条不含凭据的最近记录。
- 失败页新增可见“导出支持包”入口；第二次失败不重复建议旧网段。

### v1.5.1 (2026-08-30)
- 调整直连流程为先等待 Link UP、再修改本机网卡；未检测到链路时退出不会创建恢复任务或改动网络配置。
- 强化静态网卡恢复：写入持久静态模式，同时验证生效与持久模式，并在模式仍为 DHCP 时使用一次 WMI 静态兜底。
- 恢复快照升级至 Schema v2，记录 IPv4 地址来源并区分自动 APIPA 与手工链路本地地址；主程序、启动恢复与 watchdog 共用恢复锁。
- 修复 watchdog 参数启动时误创建交互窗口，并在重试恢复时按 GUID/MAC 重新枚举网卡。
- 更新 v1.5.1 下载页 Baby Step 教程和全套流程截图。

### v1.5.0 (2026-08-26)
- 双重确认网络修改风险，退出时只恢复本机所选网卡，不主动回退 BMC 网络设置。
- 统一主线与 Legacy 的使用教程、诊断信息和支持包说明，增加 `Alt+L` 支持包入口。
- 强化 DHCP/静态配置恢复、异常退出守护和恢复验证，降低临时配置遗留风险。
- 收紧发布 tag、构建验证、ZIP 内容检查和 R2 索引更新流程，失败时停止并保留历史记录。

### v1.3.5 (2026-07-20)
- 缓存最近一次 DHCP ACK 的租约，避免 BMC 在链路检测期间已获取地址时，界面因错过事件而误等待 3 分钟。
- DHCP 使用通配地址接收广播，并按数据包实际到达的 IPv4 接口索引过滤，只处理所选有线网卡的数据。
- 原生命令输出自动识别 UTF-8 与系统 OEM 编码，避免不同命令采用不同代码页时仍出现乱码。
- 修复诊断报告中 `ipconfig`、`route`、`netstat` 等中文系统输出乱码，并为日志和诊断 TXT 写入 UTF-8 BOM。
- 忽略明确选择了其他 DHCP Server 的 DHCPREQUEST，避免在误接局域网时干扰现有客户端。
- 恢复所选网卡使用工具前的 DHCP/静态 IPv4、网关与 DNS 配置，并要求连续两次实时验证通过。
- 增加持久化恢复快照、异常退出守护和下次启动恢复，降低强制结束或断电后遗留临时 IP 的风险。
- DHCP 分配地址后先检测 HTTPS/HTTP 管理端口，再选择协议打开浏览器；页面未就绪时保留手动打开和重新检测入口。
- 增加 DHCP 网卡绑定、Server Identifier 筛选、恢复快照、端口探测及界面布局烟雾测试。

### v1.3.4 (2026-06-28)
- Full、Lite 和 Legacy 下载改为 ZIP 压缩包，降低浏览器直接下载 EXE 时的拦截概率。
- 下载页接入 Cloudflare Pages Functions 与 KV 下载统计。

### v1.3.3 (2026-06-23)
- 将 `.NET Framework 4.6` 官方下载链接改为 Microsoft Download Center 离线安装包页面。

### v1.3.2 (2026-06-23)
- Legacy 压缩包新增 `使用教程.txt`，提示旧系统用户先安装包内 `.NET Framework 4.6` 离线安装包，再运行 Legacy 程序。

### v1.3.1 (2026-06-23)
- Legacy 发布压缩包内置 `.NET Framework 4.6` 离线安装包，减少 Windows 7/8/8.1 用户额外下载步骤。

### v1.3.0 (2026-06-23)
- 🧓 新增 Legacy 版：面向 Windows 7 SP1 / Windows 8 / Windows 8.1，基于 .NET Framework 4.6，部署时复制整个 `ezgetBMCIP-legacy-net46` 文件夹。
- 🛠️ Legacy 适配旧系统网络栈：使用 WMI / netsh / 注册表路径替代 Win10+ PowerShell cmdlet。
- 🧹 强化退出恢复 DHCP：关闭内置 DHCP Server 后恢复网卡 DHCP，Legacy 会清理 Win7/8 系列 DHCP lease/option 注册表缓存，避免接回交换机后仍保留工具分配的 IP。
- 📝 完整诊断日志：主线日志写入 `%LOCALAPPDATA%\ezgetBMCIP\ezgetBMCIP.log`，Legacy 日志写入 `%TEMP%\ezgetBMCIP.log`，应用内提供日志入口。
- 🛡️ 自定义网段限制为私有 IPv4：`10.x.x.x`、`172.16-31.x.x`、`192.168.x.x`，避免公网地址被代理/路由策略干扰。
- ✅ 已验证：Win10/11 主线 Lite 回归通过；Legacy 已在 Windows 7 SP1 和 Windows 8.1 实体机通过完整链路验证；Windows 8 按同技术路径默认兼容。

### v1.2.0 (2026-05-27)
- 🆕 自定义网段：4 输入框手动修改 IP，第 4 段自动保护 BMC 固定地址 .100
- 🆕 启动页两步化：准备 → 选网卡，Prep 卡片居中悬浮
- 🆕 IP 亮化展示：大字显示 BMC 地址 + 一键复制（2s 反馈）+ 打开 BMC
- 🆕 60s 链路超时警告：等待网线超时主动提示
- 🔧 DHCP 地址池瘦身：100~200 → 固定 .100（1:1 直连场景）
- 🐛 MAC 归一化修复：DISCOVER/REQUEST hlen 不一致导致显示 IP 比实际多 1
- 🐛 DHCP Option 50 校验：REQUEST 阶段防御性匹配客户端期望 IP
- 📝 全局文案去重、措辞统一、Hero 文字专注自身阶段
- 🎨 Step 0 内容居中 + 退出按钮降级

### v1.1.2 (2026-05-24)
- 下载页新增更新说明展示，支持 hero-notes / version-notes
- CI 自动从 git tag 注解提取完整更新日志写入 Release 和 versions.json
- README 补充界面、暗色模式说明

### v1.1.1 (2026-05-24)
- 新增自动跟随 Windows 系统亮/暗主题
- 移除 Hero 卡片多余 Logo 和呼吸动画
- 按钮改用 WPF-UI Appearance 属性

### v1.1.0 (2026-05-24)
- 迁移到 WPF-UI 4.x，全面换肤
- FluentWindow + Win11 Mica 云母材质
- 内置最小化/最大化/关闭，支持贴靠布局
- 步骤指示器改用纯 XAML DataTrigger，删除 8 个 Converter

### v1.0.7 (2026-05-24)
- 下载页资源描述优化、favicon、版本号从 CI tag 注入

### v1.0.6 (2026-05-24)
- 修复 Lite 构建、版本号读取、Full/Lite 分离
- 下载页新增最新版/历史版本双标签

### v1.0.4 (2026-05-24)
- 首个正式发布：CI/CD、R2 上传、下载页、双版本
- WinForms → WPF，.NET 8 LTS，有线网卡过滤
